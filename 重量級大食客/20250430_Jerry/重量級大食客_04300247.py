...
pygame.mixer.music.set_volume(MUSIC_VOLUME)  # 設定音量（0.0 到 1.0 範圍）
...
customer_sound.set_volume(SOUND_VOLUME)
ice_cream_sound.set_volume(SOUND_VOLUME)
wrong_sound.set_volume(SOUND_VOLUME)
coin_sound.set_volume(SOUND_VOLUME)
click_sound.set_volume(SOUND_VOLUME)
bye_bye_sound.set_volume(SOUND_VOLUME)
gameover_sound.set_volume(SOUND_VOLUME)
...
# 繪製滑桿的函數，用於設定選單的音量調整
def draw_slider(rect, pos, label, label_pos, font_label=SMALL_FONT):
    ...
    draw_text(label, label_pos[0], label_pos[1], font_label,(255, 255, 255))  # 繪製滑桿標籤

# 繪製設定畫面
def draw_settings_menu():
    global screen_stage, music_volume, sound_volume, music_slider_pos, sound_slider_pos, dragging_music_slider, dragging_sound_slider
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            dragging_music_slider = False
            dragging_sound_slider = False
        elif event.type == pygame.MOUSEMOTION and dragging_music_slider:
            new_pos = event.pos[0]
        ...
        elif event.type == pygame.MOUSEMOTION and dragging_sound_slider:
            new_pos = event.pos[0]
...
def draw_gameover():
    ...
    game_over_img = pygame.transform.scale(game_over_img, (800, 600)) # 調整背景圖尺寸
